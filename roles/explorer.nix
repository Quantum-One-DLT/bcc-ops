...
let
  ...
  nodeCfg = config.services.bcc-node;
  ogmiosCfg = config.services.bcc-ogmios;
  getSrc = name: variant.${name} or sourcePaths.${name};

  bccNodeConfigPath = builtins.toFile "bcc-node-config.json" (builtins.toJSON nodeCfg.nodeConfig);

  dbSyncPkgs = let s = getSrc "bcc-db-sync"; in import (s + "/nix") { gitrev = s.rev; };
  inherit (dbSyncPkgs) bccDbSyncHaskellPackages;
  inherit (bccDbSyncHaskellPackages.bcc-db-sync.components.exes) bcc-db-sync;
  inherit (bccDbSyncHaskellPackages.bcc-db-sync-extended.components.exes) bcc-db-sync-extended;
  inherit (bccDbSyncHaskellPackages.bcc-node.components.exes) bcc-node;
  inherit (bccDbSyncHaskellPackages.bcc-db-tool.components.exes) bcc-db-tool;

  bcc-explorer-app-pkgs = import (getSrc "bcc-explorer-app");
in {
  imports = [
    bcc-ops.modules.bcc-postgres
    bcc-ops.modules.base-service
    ((sourcePaths.bcc-db-sync-service or (getSrc "bcc-db-sync")) + "/nix/nixos")
    ((getSrc "ogmios") + "/nix/nixos")
    ((getSrc "bcc-graphql") + "/nix/nixos")
    ((getSrc "bcc-rosetta") + "/nix/nixos")
  ];

  environment.systemPackages = with pkgs; [
    bat fd lsof netcat ncdu ripgrep tree vim dnsutils bcc-cli
    bcc-db-tool
  ];

  services.bcc-postgres.enable = true;
  services.postgresql = {
  ...
  services.bcc-graphql = {
    enable = true;
    inherit bccNodeConfigPath;
    allowListPath = bcc-explorer-app-pkgs.allowList;
    metadataServerUri = globals.environmentConfig.metadataUrl or null;
    ogmiosHost = ogmiosCfg.hostAddr;
    ogmiosPort = ogmiosCfg.port;
  } // lib.optionalAttrs (options.services.bcc-graphql ? genesisCole) {
    genesisCole = nodeCfg.nodeConfig.ColeGenesisFile;
    genesisSophie = nodeCfg.nodeConfig.SophieGenesisFile;
  };

  services.bcc-rosetta-server = {
    enable = true;
    topologyFilePath = tbcoNix.bccLib.mkEdgeTopology {
      edgeNodes = map (p: p.addr) nodeCfg.producers;
      port = nodeCfg.port;
    };
    bccCliPath = bcc-cli + /bin/bcc-cli;
    genesisPath = nodeCfg.nodeConfig.SophieGenesisFile;
    bccNodePath = bcc-node + /bin/bcc-node;
    bccNodeSocketPath = nodeCfg.socketPath;
  ...
  services.bcc-db-sync = {
    ...
    logConfig = tbcoNix.bccLib.defaultExplorerLogConfig // { PrometheusPort = globals.bccExplorerPrometheusExporterPort; };
    user = "cexplorer";
    extended = globals.withBccDBExtended;
    inherit dbSyncPkgs;
    postgres = {
      database = "cexplorer";
    };
  ...
  users.users.cexplorer = {
    ...
    extraGroups = ["bcc-node"];
  };

  systemd.services.bcc-db-sync.serviceConfig = {
    # FIXME: https://github.com/The-Blockchain-Company/bcc-db-sync/issues/102
    Restart = "always";
    RestartSec = "30s";
  };

  systemd.services.bcc-ogmios.serviceConfig = {
    User = "cexplorer";
  };

  systemd.services.bcc-rosetta-server.serviceConfig = {
    User = "cexplorer";
  };

  systemd.services.bcc-graphql = {
    environment = {
      HOME = "/run/${config.systemd.services.bcc-graphql.serviceConfig.RuntimeDirectory}";
    };
    serviceConfig = {
      User = "cexplorer";
      RuntimeDirectory = "bcc-graphql";
    };
  };
  ...
  systemd.services.bcc-submit-api.serviceConfig = lib.mkIf globals.withSubmitApi {
    # Put bcc-submit-api in "bcc-node" group so that it can write socket file:
    SupplementaryGroups = "bcc-node";
  };

  services.bcc-submit-api = lib.mkIf globals.withSubmitApi {
    enable = true;
    port = 8101;
    ...
    script = ''
      ...
      pingAddr() {
        ...
        while IFS= read -r ip; do
          set +e
          PING="$(timeout 7s bcc-ping -h $ip -p $port -m $NETWORK_MAGIC -c 1 -q --json)"
          res=$?
          if [ $res -eq 0 ]; then
          ...
          if [ $res -eq 0 ]; then
            ...
            >&2 echo "failed to bcc-ping $addr:$port (on ip: $ip)"
          fi
        done <<< "$allAddresses"
      }

      run() {
        epoch=$(bcc-cli query tip --testnet-magic $NETWORK_MAGIC | jq .epoch)
        db_sync_epoch=$(psql -t --command="select no from epoch_sync_time order by id desc limit 1;")

        if [ $(( $epoch - $db_sync_epoch )) -gt 1 ]; then
          >&2 echo "bcc-db-sync has not catch-up with current epoch yet. Skipping."
          exit 0
        fi
  ...
  services.nginx = {
    ...
    virtualHosts = {
      explorer = {
        ...
        in {
          "/" = {
            root = (bcc-explorer-app-pkgs.overrideScope'(self: super: {
              static = super.static.override {
                graphqlApiHost = globals.explorerHostName;
                bccNetwork = globals.environmentName;
          ...
          "/supply/total" = {
            ...
            extraConfig = graphqlRewriter
              "{ bcc { supply { total } } }"
              "obj.data.bcc.supply.total / 1000000";
          };
          "/supply/circulating" = {
            proxyPass = "http://127.0.0.1:3100/";
            extraConfig = graphqlRewriter
              "{ bcc { supply { circulating } } }"
              "obj.data.bcc.supply.circulating / 1000000";
          };
        }) // {
          "/relays" = {
            root = "/var/lib/registered-relays-dump";
          };
          "/metrics2/bcc-graphql" = {
  ...
  services.monitoring-exporters.extraPrometheusExporters = [
    # TODO: remove once explorer exports metrics at path `/metrics`
    {
      ...
      scrape_interval = "10s";
      port = globals.bccExplorerPrometheusExporterPort;
      metrics_path = "/";
      labels = { alias = "${name}-exporter"; };
    }
  ] ++ lib.optional config.services.bcc-graphql.enable
    {
      job_name = "bcc-graphql-exporter";
      scrape_interval = "10s";
      metrics_path = "/metrics2/bcc-graphql";
      labels = { alias = "bcc-graphql-exporter"; };
    };
}
