...
{
  imports = [
    pkgs.bcc-ops.modules.load-client
  ];

  systemd.services.bcc-node.after = [ "ephemeral.service" ];

  # Configure high IOPS on any ec2 node supporting it for bcc-node load client
  systemd.services.ephemeral = {
    wantedBy = [ "multi-user.target" ];
    before = [ "bcc-node.service" "sshd.service" ];
    serviceConfig = {
      Type = "oneshot";
    };
    path = with pkgs; [
