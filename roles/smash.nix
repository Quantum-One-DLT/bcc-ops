...
in {
  environment = {
    systemPackages = [
      bat fd lsof netcat ncdu ripgrep tree vim bcc-cli
    ];
    variables = {
  ...
  services.bcc-node = {
    ...
    package = smashHaskellPackages.bcc-node.components.exes.bcc-node;
  ...
  systemd.services.smash.serviceConfig = {
    # Put bcc-db-sync in "bcc-node" group so that it can write socket file:
