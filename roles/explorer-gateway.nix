...
in {
  ...
  imports = [
    (bcc-ops.roles.explorer globals.explorerDbSnapshots)
  ];

  # Disable services not necessary for snapshots:
  services.bcc-ogmios.enable = lib.mkForce false;
  services.graphql-engine.enable = lib.mkForce false;
  services.bcc-graphql.enable = lib.mkForce false;
  services.bcc-rosetta-server.enable = lib.mkForce false;
  services.bcc-submit-api.enable = lib.mkForce false;
  services.nginx.enable = lib.mkForce false;

  # Create a new snapshot every 24h (if not exist alreay):
  services.bcc-db-sync.takeSnapshot = "always";
  systemd.services.bcc-db-sync.serviceConfig.RuntimeMaxSec = 24 * 60 * 60;
  ...
  services.traefik = {
    ...
    staticConfigOptions = {
      ...
      entryPoints = {
        ...
        metrics = {
          address = ":${toString globals.bccExplorerGwPrometheusExporterPort}";
        };
      };
  ...
  services.monitoring-exporters.extraPrometheusExporters = [
    {
      ...
      port = globals.bccExplorerGwPrometheusExporterPort;
    }
  ];
  ...
  in
    ...
    in {
      ...
      config = {
        ...
        nixpkgs.pkgs = pkgs;
        imports = [(bcc-ops.roles.explorer variant)];
