...
let
  ...
  inherit (tbco-ops-lib.physical) aws;

  cluster = import ../clusters/bcc.nix {
    inherit pkgs;
  ...
  settings = {
    resources = {
      ...
      ec2KeyPairs = listToAttrs (concatMap (region:
        map (org:
          nameValuePair "bcc-keypair-${org}-${region}" {
            inherit region;
    ...
    defaults = { name, resources, config, ... }: {
      deployment.ec2 = {
        keyPair = resources.ec2KeyPairs."bcc-keypair-${config.node.org}-${config.deployment.ec2.region}";
