...
then
    ...
    case $1 in
        redeploy )
            ...
            nixops destroy
            ./scripts/create-sophie-genesis-and-keys.sh
            nixops deploy -k
            ;;
        * )
...
for f in ${BFT_NODES[@]}
do
    nixops scp $f examples/sophie-testnet/scripts/submit-update-proposal.sh /root/ --to
done

for f in ${POOL_NODES[@]}
do
    nixops scp $f examples/sophie-testnet/scripts/register-stake-pool.sh /root/ --to
done
