...
class KesRotate
  ...
  def initialize
    ...
    if scriptCmdPrivate("nix-instantiate --eval -E --json 'let n = import #{PATH_MOD}/deployments/bcc-aws.nix; in __attrNames n'").success?
  ...
  def doRotation
    ...
    IO_TEE_OUT.puts "LATEST_BCC_URL: #{LATEST_BCC_URL}"
    ...
    if scriptCmdPrivate("curl -sL #{LATEST_BCC_URL} | grep https | grep download | grep -oP '=\"\\K[^\"]+'").success?
    ...
    case protocol
    when "RealPBFT" then kesAbort("KES rotation does not need to happen in the cole era")
    when "TOptimum"   then eraName = "sophie"
    when "Bcc"  then eraName = "sophie"
    ...
    kesNodes.as(Array).each do |n|
      if scriptCmdPrivate("nixops ssh #{n} -- 'curl -s #{n}:#{NODE_METRICS_PORT}/metrics | grep -oP \"bcc_node_metrics_slotNum_int \\K[0-9]+\"'").success?
    ...
    kesNodes.each do |n|
      if MOCK_ENABLED
        ...
        rotateCmd3 = "nixops ssh-for-each --include #{n} -- 'systemctl restart bcc-node'"
      end
      ...
      IO_TEE_OUT.puts "Restarting bcc-node service on core node: #{n} (#{rotateCmd3})"
      if scriptCmdPrivate(rotateCmd3).success?
        IO_TEE_OUT.puts IO_CMD_OUT.to_s
        IO_TEE_OUT.puts IO_CMD_ERR.to_s
      else
        kesAbort("Failed to restart bcc-node with the new KES key on core node: #{n} (#{rotateCmd3})")
      end
