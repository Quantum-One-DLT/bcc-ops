...
class DbSyncSnapshot
  ...
  def updateAbort(msg)
    msg = "Bcc-db-sync snapshot upload aborted on #{@cluster} at #{NOW}:\n" \
          "MESSAGE: #{msg}\n" \
          "STDOUT: #{IO_CMD_OUT}\n" \
          "STDERR: #{IO_CMD_ERR}"
    if (@emailOpt != "")
      sendEmail("Bcc-db-sync snapshot upload ABORTED on #{@cluster} at #{NOW}", "#{msg}\n\nFULL LOG:\n#{IO_TEE_FULL}")
    else
      IO_TEE_OUT.puts msg
    end
    exit(1)
  ...
  def uploadSnapshot(snapshotFile)
    ...
    if matchMajorVersion == nil
      ...
      if !runCmdVerbose("./scripts/upload-with-checksum.sh #{SNAPSHOT_WORK_DIR}/#{snapshotFile} #{@s3Bucket} bcc-db-sync/#{majorVersion}").success?
        updateAbort("Error while upload db-sync snasphot.")
      end
      uploadLog = IO_CMD_OUT.to_s
      runCmdVerbose("rm -f #{SNAPSHOT_WORK_DIR}/#{snapshotFile}")
      uploadLog
    end
  end
  ...
  def run()
    ...
    IO_TEE_OUT.puts "Bcc-db-sync snapshot upload on cluster #{@cluster} at #{NOW}, completed."
    if (@emailOpt != "")
      sendEmail("Bcc-db-sync snapshot upload on cluster #{@cluster} at #{NOW}, completed.", uploadLog)
    end
  end
end
