...
class RelayUpdate
  ...
  def takeDbSnapshot(fromNode : String)
    if !runCmdVerbose("nixops ssh #{fromNode} "\
      "'cd /var/lib/bcc-node && systemctl stop bcc-node "\
      "&& mv db-#{@cluster}.tar.gz db-#{@cluster}.tar.gz.bak || : "\
      "&& tar czf db-#{@cluster}.tar.gz db-#{@cluster} && systemctl start bcc-node'").success?
      updateAbort("Could not snapshot node db on #{fromNode}.")
    end
    targetFile = "db-#{@cluster}-#{Time.utc.to_s("%Y-%m-%d")}.tar.gz"
    if !runCmdVerbose("nixops scp --from #{fromNode} /var/lib/bcc-node/db-#{@cluster}.tar.gz #{targetFile}").success?
  ...
  def doUpdate
    ...
    if runCmdVerbose("nix eval --json '(__attrNames (import #{PATH_MOD}/deployments/bcc-aws.nix))'").success?
      network = Array(String).from_json(IO_CMD_OUT.to_s)
    else
      updateAbort("Unable to process the attribute names from the deployment.")
    end

    if runCmdVerbose("nix eval --json '(__attrNames (import #{PATH_MOD}/deployments/bcc-aws.nix).resources.ec2SecurityGroups)'").success?
    ...
    if runCmdVerbose("nix eval --json '(__attrNames (import #{PATH_MOD}/deployments/bcc-aws.nix).resources.elasticIPs)'").success?
    ...
    if runCmdVerbose("nix eval --json '(__attrNames (import #{PATH_MOD}/deployments/bcc-aws.nix).resources.route53RecordSets)'").success?
      ...
      if !batchTargetNodes.empty?
        ...
        if (@snapshotDbOpt != "" && !@mockOpt)
          ...
          batchTargetNodes.each do |n|
            if !runCmdVerbose("nixops scp --to #{n} #{snapshot} /var/lib/bcc-node/db-restore.tar.gz").success?
        ...
        batchTargetNodes.each do |n|
          ...
          if @mockOpt
            ...
            while i < METRICS_WAIT_ITERATIONS
              if runCmdVerbose("nixops ssh #{n} -- 'curl -s #{n}:#{NODE_METRICS_PORT}/metrics | grep -oP \"bcc_node_metrics_slotNum_int \\K[0-9]+\"'").success?
