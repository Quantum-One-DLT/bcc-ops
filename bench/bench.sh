...
. "$__BENCH_BASEPATH"/lib-genesis-cole.sh
...
### TODO
###
##  1. Automate genesis using bcc-node's Nix code. Param details are key.
...
USAGE:  $(basename "$0") OPTIONS.. [OP=bench-profile] OP-ARGS..
  ...
  Main OPs:
    ...
    reinit-params         Update ${paramsfile} for current 'bcc-ops'.
...
usage_extra() {
  ...
  Other OPs:
    ...
    grep                  For each host:  grep "\$1" /var/lib/bcc-node/logs/*
...
EOF
                ...
                grep )                local exp=${1?Usage:  grep EXPR}; shift
                                      nixops ssh-for-each --parallel -- grep "'$exp'" "/var/lib/bcc-node/logs/*" "$@";;
...
op_bench_start() {
        ...
        nixops ssh-for-each --parallel "rm -rf /var/log/journal/* /var/lib/bcc-node/{db*,logs,logs-*,*.log,utxo}"

        oprint "$(date), restarting journald & nodes.."
        nixops ssh-for-each --parallel "systemctl start systemd-journald"
        sleep 3s
        nixops ssh-for-each --parallel "systemctl start bcc-node"
        ...
        sources_json_node_commit=$(jq '.["bcc-node"].rev' \
                                      nix/sources.bench.json --raw-output)

        if ! nixops ssh "explorer" -- systemctl status bcc-node >/dev/null
        ...
        local canary='node-0'
        if ! nixops ssh "$canary" -- systemctl status bcc-node >/dev/null
        ...
        if nixops ssh "$canary" -- journalctl -u bcc-node |
...
fetch_systemd_unit_startup_logs() {
        ...
        nixops ssh explorer "journalctl --boot 0 -u bcc-node | head -n 100" \
          > 'logs/startup/unit-startup-explorer.log'

        for node in $(params producers)
        do nixops ssh "${node}" "journalctl --boot 0 -u bcc-node | head -n 100" \
...
{ meta:
  ...
  , node_commit_desc:  \"$(git_local_repo_query_description \
                              'bcc-node' \
                              $(depljq explorer '.pins["bcc-node"]' --raw-output))\"
...
op_wait_for_nonempty_block() {
        ...
        do r=$(nixops ssh explorer -- sh -c "'tac /var/lib/bcc-node/logs/node.json | grep -F MsgBlock | jq --compact-output \"select(.data.msg.\\\"txIds\\\" != [])\" | wc -l'")
           if test "$r" -ne 0
           then l=$(nixops ssh explorer -- sh -c "'tac /var/lib/bcc-node/logs/node.json | grep -F MsgBlock | jq \".data.msg.\\\"txIds\\\" | select(. != []) | length\" | jq . --slurp --compact-output'")
                echo ", got $l, after $((now - since)) seconds"
                return 0; fi
           e=$(nixops ssh explorer -- sh -c \
                   "'tac /var/lib/bcc-node/logs/node.json | grep -F MsgBlock | jq --slurp \"map (.data.msg.\\\"txIds\\\" | select(. == [])) | length\"'")
...
op_wait_for_empty_blocks() {
        ...
        do while news=$(nixops ssh explorer -- sh -c "'set -euo pipefail; { echo \"{ data: { msg: { blkid: 0, txIds: [] }}}\"; tac /var/lib/bcc-node/logs/node.json; } | grep -F MsgBlock | jq --compact-output \".data.msg | { blkid: (.blockHash | ltrimstr(\\\"\\\\\\\"\\\") | rtrimstr(\\\"\\\\\\\"\\\")), tx_count: (.txIds | length) } \"'" |
...
fetch_run() {
        ...
        do nixops ssh "${mach}" -- \
             "cd /var/lib/bcc-node &&
              ...
              (test ! -f bcc-node.prof ||
               mv bcc-node.prof ${mach}.prof;) &&
              (test ! -f bcc-node.eventlog ||
               mv bcc-node.eventlog ${mach}.eventlog;) &&
              ( journalctl -ru bcc-node |
                head -n 90 | cut -d: -f4 |
                sed -n '/  \]/,/bytes allocated/ p' |
                tac > ${mach}-bcc-node-gcstats.log; ) &&
