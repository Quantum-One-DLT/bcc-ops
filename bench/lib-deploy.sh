...
update_deployfiles() {
        ...
        jq >"${files[0]}" "
          ...
          , pins:
            { \"bcc-node\":          $(jq '.["bcc-node"].rev'         nix/sources.bench.json)
            , \"bcc-ops\":           \"$(git rev-parse HEAD)\"
            }
...
deploystate_node_process_genesis_startTime() {
        ...
        local node_process
        node_process=$(nixops ssh ${core} -- pgrep -al bcc-node)

        local genesis
        if test -n "$node_process"
        then genesis=$(nixops ssh ${core} -- jq . \
                     $(nixops ssh ${core} -- jq .SophieGenesisFile \
                     $(sed 's_.* --config \([^ ]*\) .*_\1_' <<<$node_process)))
             case $(get_era) in
                     sophie ) jq '.systemStart
                                  | fromdateiso8601
                                  '  --raw-output <<<$genesis;;
             esac
        else fail "bcc-node isn't running"; fi
}
...
deploystate_create() {
        nixops create ./deployments/bcc-aws.nix -I nixpkgs=./nix
}
...
deploystate_deploy_profile() {
        ...
        era=$(get_era)
        topology=$(parmetajq .topology)
        node_rev=$(jq --raw-output '.["bcc-node"].rev' nix/sources.bench.json)
...
EOF
        ...
        host_resources=( $(nixops info --plain 2>/dev/null | sed 's/^\([a-zA-Z0-9-]*\).*/\1/' | grep -ve '-ip$\|bcc-keypair-\|allow-'))
        other_resources=($(nixops info --plain 2>/dev/null | sed 's/^\([a-zA-Z0-9-]*\).*/\1/' | grep  -e '-ip$\|bcc-keypair-\|allow-'))
        set -o pipefail
...
deploystate_node_log_commit_id() {
        ...
        set +o pipefail
        nixops ssh "$mach" -- journalctl -u bcc-node | grep commit | tail -n1 | sed 's/.*"\([0-9a-f]\{40\}\)".*/\1/'
        set -o pipefail
}
...
op_stop() {
        nixops ssh-for-each --parallel "systemctl stop bcc-node 2>/dev/null || true" &
...
op_jq_generator() {
        local final_jq_opts=(
                "${remote_jq_opts[@]}"
                "'${1:-.}'"
                "/var/lib/bcc-node/logs/generato*.json"
        )
        if ! op_on 'explorer' ls '/var/lib/bcc-node/logs/generato*.json' >/dev/null
...
op_blocks() {
        nixops ssh explorer 'jq --compact-output "select (.data.kind == \"Recv\" and .data.msg.kind == \"MsgBlock\") | .data.msg" /var/lib/bcc-node/logs/node-*.json'
}
...
op_fetch_utxo() {
    ...
    op_on $node bcc-cli query utxo --bcc-mode --whole-utxo --testnet-magic 42 --out-file '/var/lib/bcc-node/utxo'
    oprint "fetching UTxO from $node into $file.."
    nixops scp --from $node '/var/lib/bcc-node/utxo' "$file"
}
