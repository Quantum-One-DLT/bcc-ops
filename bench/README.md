# Bcc benchmarking infrastructure
...
The benchmarking infrastructure consists of the following components:
  ...
  - The `bcc-benchmarking` repository, which contains:
      ...
      - the `bcc-tx-generator`, that generates Tx load on arbitrary
        nodes,
  ...
  - Resources on the development deployer, available under the `dev`
    user:
      ...
      - A number of cluster deployments, that have `bcc-ops`
...
1.  Ensure the cluster params file (`benchmarking-cluster-params.json`)
    ...
    This step is only necessary when `bcc-ops` is updated in a major
...
1.  **Source pins** for the components (`bcc-node`,
    `bcc-db-sync` and `bcc-benchmarking` repositories).
...
5.  **Deployment checkout** is a per-cluster checkout of the
    `bcc-ops` repository, that is situated in the home directory of
...
**WARNING 1**: it is strongly discouraged to edit the `bcc-ops`
...
remote for the `bcc-ops` deployment checkout, and use `git` to push
...
2.  Topology and size
    ...
    `bcc-ops`, and are called `bench-TYPE-N.nix`, where N is
    ...
    1.  ``` example
        ...
        `bcc-db-sync` service definition issue, and then:
...
1.  Log fetching
    ...
    The `bcc-db-sync` database has the SQL extraction queries
    performed on it. (For the queries, please look for the
    `scripts/NN-*.sql` files in the `bcc-benchmarking` repository).
...
2.  Analysis
    ...
    of them coming from the `bcc-benchmarking` repository (in the
...
1.  A run output directory, such as:
    ...
    8.  `tools/*` – the tools used to perform some of the analyses,
        fetched from the `bcc-benchmarking` repo.
