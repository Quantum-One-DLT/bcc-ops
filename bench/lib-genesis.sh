...
profile_genesis() {
        ...
        profile_genesis_cole "$profile" "$genesis_dir/cole"

        genesis_update_starttime       "$start_timestamp" "$genesis_dir"
        genesis_update_starttime_cole "$start_timestamp" "$genesis_dir/cole"

        local hash hash_cole
        hash=$(genesis_hash "$genesis_dir")
        echo -n "$hash"> "$genesis_dir"/GENHASH

        hash_cole=$(genesis_hash_cole "$genesis_dir/cole")
        echo -n "$hash_cole"> "$genesis_dir/cole"/GENHASH
...
cli() {
        echo "---)  bcc-cli $*" >&2
        bcc-cli "$@" || fail "cli invocation failed"
}
...
profile_genesis_singleshot() {
        ...
        jq '
          ...
          }' --null-input > "$target_dir"/aurum-genesis.json

        genesis_remap_key_names "$prof" "$ids_pool_map"

        ## Fix up the key, so the generator can read it:
        sed -i 's_PaymentSigningKeySophie_SigningKeySophie_' "$target_dir"/utxo-keys/utxo1.skey
}
...
genesis_hash() {
        ...
        bcc-cli genesis hash --genesis "${genesis_dir}"/genesis.json |
                tr -d '"'
}
